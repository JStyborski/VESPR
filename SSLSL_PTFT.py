...
from SSL_Pretrain import init_ssl_loss, load_checkpoint, adjust_lr_per_epoch
...
def get_args_parser():
    ...
    parser.add_argument('--linClsSG', default=False, type=lambda x:bool(strtobool(x)), help='Boolean to apply stop-gradient before linear classifier')
    ...
    parser.add_argument('--lossWtAlpha', default=1.0, type=float, help='Weighting coefficient on SSL loss')
    parser.add_argument('--lossWtBeta', default=1.0, type=float, help='Weighting coefficient on SL loss')
    ...
    parser.add_argument('--advLossWtAlpha', default=0.0, type=float, help='Weighting coefficient on SSL loss')
    parser.add_argument('--advLossWtBeta', default=1.0, type=float, help='Weighting coefficient on SL loss')
...
def init_model(args):
    ...
    model = Models.PTFT_Model(args.encArch, args.rnCifarMod, args.vitPosEmb, args.vitPPFreeze, args.prjArch, args.prjHidDim, args.prjBotDim, args.prjOutDim,
                              args.prdHidDim, args.prdAlpha, args.prdEps, args.prdBeta, args.momEncBeta, args.applySG, args.linClsSG, args.nClasses)
...
def init_optimizer(args, model):
    ...
    if args.useDDP:
        optimParams = [{'params': model.module.encoder.parameters(), 'decayLR': args.decayEncLR},
                       ...
                       {'params': model.module.lincls.parameters(), 'decayLR': args.decayEncLR}]
    else:
        optimParams = [{'params': model.encoder.parameters(), 'decayLR': args.decayEncLR},
                       ...
                       {'params': model.lincls.parameters(), 'decayLR': args.decayEncLR}]
...
def train_one_batch(args, batch, model, lossFn1, lossFn2, lossScaler, optimizer):
    ...
    if any(args.useAdvList):
        ...
        _, advTensList = FGSM_PGD.ssl_sl_pgd(model, lossFn1, lossFn2, args.advLossWtAlpha, args.advLossWtBeta, args.useAMP, augList, truthTens,
                                             args.useAdvList, args.gatherTensors, args.advAlpha, args.advEps, args.advNorm, args.advRestarts,
                                             args.advSteps, args.advBatchSize, targeted=False, randInit=args.advNoise, noiseMag=args.advNoiseMag,
                                             xMin=args.advClipMin, xMax=args.advClipMax)
    ...
    with torch.cuda.amp.autocast(enabled=args.useAMP):
        ...
        for augTens in augList:
            ...
            if args.gatherTensors:
                ...
                c = torch.cat(MF.FullGatherLayer.apply(c.contiguous()), dim=0)

            # Append to lists for loss calculation
            outList1.append([p, z, r, mz])
            outList2.append(c)

        # Gather targets to match gathered predictions
        if args.nProcs > 1 and args.gatherTensors:
            truthTens = torch.cat(MF.FullGatherLayer.apply(truthTens.contiguous()), dim=0)

        # Calculate losses
        lossVal1 = lossFn1(outList1)
        lossVal2, nCorrect, nTotal = lossFn2(outList2, truthTens)
        lossVal = args.lossWtAlpha * lossVal1 + args.lossWtBeta * lossVal2
    ...
    return lossVal, lossVal1, lossVal2, nCorrect, nTotal, outList1
...
def save_checkpoint(args, epoch, model, lossScaler, optimizer):
    ...
    chkptPath = 'Trained_Models/{}_ptft_{:04d}.pth.tar'.format(args.filePrefix, epoch)
    if args.gpu == 0:
...
def sslsl_ptft_main():
    ...
    if args.useDDP or args.nProcs > 1:
        ...
        torch.multiprocessing.spawn(sslsl_ptft_worker, nprocs=args.nProcs, args=(args,))
    # Launch one process for main_worker function
    else:
        sslsl_ptft_worker(args.gpu, args)

def sslsl_ptft_worker(gpu, args):
    ...
    lossFn1 = init_ssl_loss(args)
    lossFn2 = Losses.MultiAug_CrossEnt_Loss(args.symmetrizeLoss)
    ...
    for epoch in range(args.startEpoch, args.nEpochs + 1):
        ...
        for batchI, batch in enumerate(trainDataLoader):
            ...
            lossVal, lossVal1, lossVal2, nCorrect, nTotal, outList = train_one_batch(args, batch, model, lossFn1, lossFn2, lossScaler, optimizer)

            if batchI + 1 >= args.nBatches:
                break

        print('Epoch: {} / {} | CurLR: {:0.4f} | Time: {:0.2f} | Last SSL Loss: {:0.4f} | Last SL Loss: {:0.4f} | Last Train Acc: {:0.4f}'
              .format(epoch, args.nEpochs, optimizer.param_groups[0]['lr'], time.time() - timeStart, lossVal1.detach(), lossVal2.detach(), nCorrect / nTotal))
